...
library(leaflet)
library(dplyr)
adressRegion <- adressRegion|>select(-pays)
...
villes_min_VarMoyAb <- indicateurs%>%
  group_by(SPECIFICITE) %>%
  filter(min_rank(VarMoyAbs) == 1)
villes_min_VarMoyAb 
...
# Créer une carte Leaflet
 leaflet() %>%
  ...
  addPolygons(data = dpt2,
              ...
              fillOpacity = 0.2,
              stroke = FALSE) %>%
  addCircleMarkers(data = villes_min_VarMoyAb,
                   lng = ~long,
                   lat = ~lat,
                   popup = ~paste(VILLE, " - ", c(SPECIFICITE), "<br>VarMoyAbs: ", VarMoyAbs),
                   color = "red",
                   radius = 5)
...
villes_agg <- villes_min_VarMoyAb %>%
  group_by(VILLE, lat, long) %>%
  summarise(SPECIFICITE = paste(SPECIFICITE, collapse = ", "))
