...
priceGlob_visuel_long <- priceGlob_visuel_m %>%
  pivot_longer(cols = -c(PRODUITS, DATE), 
               names_to = "VILLE",        
               values_to = "PRIX")  
sum(is.na(priceGlob_visuel_long$PRIX))
sum(is.na(priceGlob$PRIX))
# Afficher le tableau en format long résultant
print(priceGlob_visuel_long)

# Suppression de la colonne PRIX de priceGlob
priceGlob <- priceGlob %>%
  select(-PRIX)

priceGlob <- full_join(priceGlob,priceGlob_visuel_long,by = c("PRODUITS", "DATE", "VILLE"))
print(priceGlob)

# Définir le chemin du répertoire du shiny
setwd("../ShinyApps_Bah/data")
write_csv(priceGlob, "priceGlob.csv")




```{r}
#Trier le jeu de données par PRODUITS et DATE
priceGlob_visuel_m <- priceGlob_visuel_m |>
  arrange(PRODUITS, DATE)
print(priceGlob_visuel_m)

# Remplir les valeurs manquantes avec les valeurs précédentes dans le cadre de chaque groupe de PRODUITS
priceGlob_visuel_m <- priceGlob_visuel_m |>
  group_by(PRODUITS) |>
...
```{r}
 ...
 ggplot( dpt2) +geom_sf( fill = "white") +
          theme_void()
```
