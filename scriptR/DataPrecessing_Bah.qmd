...
matching <- apply(DD, 2, function(x) {
  ...
  region$value[ordered_index]  # Get the label of region$value based on the ordered index
}) |> as_tibble()
names(matching) <- region$value
# Remove the first row from the matching tibble
matching <- matching[-1, ]
print(matching)
...
for (i in 1:nrow(priceGlob_visuel)) {
  # Vérifier si le prix à Abidjan est NA
  
  if (is.na(priceGlob_visuel$SANPEDRO[i])) {
    # Trouver la ville la plus proche avec un prix non nul
    for (ville_proche in matching$SANPEDRO) {
      if (!is.na(priceGlob_visuel[[ville_proche]][i])) {
        priceGlob_visuel$SANPEDRO[i] <- priceGlob_visuel[[ville_proche]][i]
        break
      }else{
        priceGlob_visuel$SANPEDRO[i]=250
      }
    }
  }
}
