...
Les prix initialement capturés sur 6 villes de la côte d'ivoire, sont à présent repartis sur tout l'ensemble du territoire ivoirien. il ne s'agira pas seulement des prix, mais de tout indicateur calculé dans la perspertive de l'analyse.
Pour cela la correspondance de chaque ville par rapport à sa ville la plus proche est alors appliquée.Environ 3658600 en tout seront générées dans un nouveau dataset.
...
priceGlobClean <- priceGlob_visuel_m |> pivot_longer(!(PRODUITS:DATE),names_to = "VILLE", values_to = "PRIX")
...
## Variation des prix par date, par produit
```{r}
# A une date pour un produit un seul prix est défini
# Dataset de base
priceGlobClean <- priceGlobClean |>
  group_by(PRODUITS,VILLE) |>
  mutate(PRIXPREC=lag(PRIX,order_by = DATE)) # Dataset de base
  # Indicateur
priceGlobClean |>
  mutate(TauxVar=round((PRIX-PRIXPREC)/PRIXPREC,2)) |>
  group_by(DATE) |>
  summarise(PrixMoy=mean(PRIX,na.rm = TRUE),
            VarMoy=mean(TauxVar,na.rm =TRUE))|>
  ## Plot
  ggplot()+
  aes(x=DATE,y=PrixMoy)+
  geom_line()+
  theme_minimal()
```

## Variation des prix par date, par produit
Nous utilisons ici les dates format Mois-an. 
```{r}
# A une date pour un produit un seul prix est défini
# Dataset de base
priceGlobClean |>
  mutate(MOIS=as.yearmon(DATE, "%m/%Y")) |> 
  group_by(PRODUITS) |> 
  mutate(PRIXPREC=lag(PRIX,order_by = DATE,default = NA)) |> 
  mutate(TauxVar=round((PRIX-PRIXPREC)/PRIXPREC,2)) |>
  group_by(MOIS) |> 
   summarise(PrixMoy=mean(PRIX,na.rm = TRUE),
            VarMoy=mean(TauxVar,na.rm =TRUE)) |> ungroup() |> 
  ggplot()+
  aes(x=MOIS,y=VarMoy)+
  geom_line()+
  theme_minimal()
...
```{r}
priceGlob |> filter(PRIX==0)
```


## Par produits selon les villes
```{r}
# A une date pour un produit un seul prix est défini
priceGlobClean |>group_by(PRODUITS,VILLE) |>   mutate(PRIXPREC=lag(PRIX,order_by = DATE)) |> mutate(TauxVar=round((PRIX-PRIXPREC)/PRIXPREC,2)) |> ungroup() |> filter(TauxVar==Inf)
```
