...
priceGlobCleanFull |>
  ...
  summarise(PrixMoy=mean(PRIX,na.rm = TRUE),
            VarMoy=mean(TauxVar,na.rm =TRUE),
            VarMoyAbs=mean(abs(TauxVar),na.rm =TRUE)) |> 
  select(QuarterAn,PrixMoy) |> 
  dygraph() |> 
  addShading(SemestreTable) |> 
   dyAnnotation("2021-7-1", text = "Produits frais", tooltip = "Korea",attachAtBottom = TRUE, width = 60)
...
```{r}
addShading <- function(dygraph,tabl){
...
```{r}
SemestreTable <- SemestreTable |> mutate(Date=as.Date(fin)) |> mutate(Quarter=as.yearqtr(Date)) 
```
